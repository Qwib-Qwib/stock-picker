def stock_picker (prices_array) #This method is the intializer. It indicates which buying date we're currently considering. It calls another method to calculate the gains and losses with that date in mind.
  i = 0
  prices_array.reduce({}) do |initial_day, current_buy_price|
    initial_day["Buy on day #{prices_array.index(current_buy_price) + 1}"] = sell_values_calculator(prices_array, buy_date_index = prices_array.index(current_buy_price))
  i += 1
  initial_day
  end
end
...
def sell_values_calculator (prices_array, buy_date_index)
  ...
  prices_array.reduce({}) do |potential_gains_and_losses, current_sell_price|
    if i <= buy_date_index  #This condition excludes all selling values for days before the current buying date.
      ...
      potential_gains_and_losses["Sell on day #{i + 1}"] = prices_array[buy_date_index] - current_sell_price
      i += 1
    end
